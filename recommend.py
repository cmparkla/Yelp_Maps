...
def group_by_centroid(restaurants, centroids):
    ...
    #     a += [[find_closest(restaurant_location(restaurant), centroids), restaurant]]
...
def find_centroid(restaurants):
    ...
    for restaurant in restaurants:
        ...
        b += [restaurant_location(restaurant)]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        linkedrescent = group_by_centroid(restaurants, centroids)
        centroids = [find_centroid(cluster) for cluster in linkedrescent]
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    Sxy = sum([(item[0] - mean(xs))*(item[1] - mean(ys)) for item in zip(xs, ys)])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for x in feature_fns:
        predictionary = find_predictor(user, reviewed, x)
        dictionary[predictionary[0]] = predictionary[1]
...
def rate_all(user, restaurants, feature_functions):
    ...
    for name in restaurants:
        if name in reviewed:
            newdic[name] = user_rating(user, name)
        else:
            newdic[name] = predictor(restaurants[name])
